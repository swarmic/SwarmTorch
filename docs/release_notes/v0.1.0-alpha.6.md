...
- Sparse decompression now ignores out-of-bounds indices instead of risking panic. *(Note: This behavior was changed to fail-closed in Wave 1 — see `v0.1.0-alpha.6x-wave1.md`.)*
...
| `decompress_sparse_ignores_out_of_bounds_indices` | core/compression | Ignore invalid sparse indices safely *(replaced by `decompress_sparse_returns_error_for_out_of_bounds_indices` in Wave 1)* |
...
cargo test --workspace                                              # ✅ passed (test counts evolve as follow-up hardening lands)
