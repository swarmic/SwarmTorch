...
- **Invalid fingerprint hex rejection:** Input fingerprints must be valid 64-char lowercase hex; non-hex strings are rejected at materialization time
...
```rust
pub fn derived_source_fingerprint_v0(asset_key: &str) -> String;
pub fn no_schema_hash_v0() -> String;
...
**What:** `materialize_node_outputs()` hardened with six pre-validation checks:
...
| Input fingerprint invalid hex | `InvalidInput` | `invalid fingerprint for input {key}` |
...
- Lineage edges count (3 edges: source→enriched, source→enriched_unsafe, enriched→report)
...
pub fn predict_output_fingerprints(
    node: &NodeV1,
    upstream_fingerprints: &[(String, String)],
...
| `union` | Combines all input assets |
...
|------|--------|----------|
| No `unwrap`/`expect` in production | ✓ | All production error paths use `Result` + `?`; `unwrap_or_default()` only on infallible `Duration` |
...
cargo test --workspace                                              # 28 pass (swarm-torch)
