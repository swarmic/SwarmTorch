...
Status: Partially completed (`A6X-01..A6X-05` landed; `A6X-06` deferred)  
...
| `A6X-06` Artifact module decomposition | Pending (deferred) | Deferred until after v0.2 F-track (`F1`, then grouped `F2a/F2/F3/F4`) |
...
10. `swarm-torch/src/artifacts.rs` is oversized (~120KB / ~2900 LOC), which raises regression risk and slows review velocity; schedule a behavior-preserving module split after correctness slices stabilize.
...
| A6X-06 | Artifact module decomposition (behavior-preserving) | `swarm-torch/src/artifacts.rs`, `swarm-torch/src/report.rs` | Split artifact/session/write-path logic into smaller modules without changing artifact schema/wire format/API; existing run bundles and `artifact_pipeline` behavior remain compatible. |
